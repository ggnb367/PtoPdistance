...
- 变化点（满足你的需求）：
  ...
  * 其余步骤（叶内 PLL、叶 rep_graph 完全图、父层 index_graph、AB/SAME 表、严格查表）保持兼容。
...
def build_index_graphs_for_parents(
    ...
    """
    gateway-only（仅网关）策略：
      - index_graph 的节点 = parent 下各 child 内“会跨 child 相邻”的代表点（网关）。
      - child 内部：对网关节点做 kNN（基于 child 的 rep_graph 距离）。
      - child 之间：保留原图中的跨 child 边。
    返回：index_graphs, stats（包含 parent 的 border 数与 index 节点数）
    """
    index_graphs = {}
    stats = {}
    ...
    for parent in tqdm(parents, desc="Step3: building index_graphs (parents)", unit="parent"):
        ...
        # child -> 边界并集（leaf rep_graph 的节点并集）
        child_to_nodes = {}
        node_to_child = {}
        for ch in children:
        ...
        # child 内部：在该 child 的 rep_graph 上做 kNN（基于最短路距离）
        ...
        # 统计
        child_border_union = set().union(*[child_to_nodes[ch] for ch in children]) if children else set()
        stats[parent] = dict(
            index_nodes=IG.number_of_nodes(),
            index_edges=IG.number_of_edges(),
            gateways=sum(len(s) for s in gateway_by_child.values()),
            parent_border_count=len(child_border_union)  # parent 的 border = 子簇边界点并集
        )
...
def main():
    ...
    # NOTE: 代表图已是“边界点完全图”，leaf_rg_k 为占位
    ap.add_argument("--leaf_rg_k", type=int, default=24, help="(占位) 叶子 rep_graph kNN 数（当前为完全图，此参数不生效）")
    ap.add_argument("--ig_intra_knn", type=int, default=24, help="父层 index_graph 各子簇网关间 kNN 边数")
    ...
    # 3) 父层 index_graph（网关收缩 + 子簇内网关 kNN）
    ...
    # === 输出：每个 parent 的 index 节点数 & parent 的 border 数 ===
    ...
    # 4) list_indexpath
    list_indexpath_lvl = build_list_indexpath_for_parents(
        cluster_tree, index_graphs_lvl, rep_graphs, parents_subset=None
    )

    # 7) AB（float32）
    AB_tables_lvl = build_AB_tables(cluster_tree, rep_graphs, list_indexpath_lvl, mid_limit=args.mid_limit)
    ...
    L2B, L2C, leaf_row_index = build_leaf2pair_tables_mixed(
        cluster_tree, borders, G_nb_all, SAME_tables, AB_tables_lvl,
        dtype=args.l2c_dtype, mode=args.l2_mode, union_ratio=args.l2_union_ratio
    )
    tL2b = time.time()
    preprocessing_time = tL2b - tALL0
    ...
    df_eval = evaluate_all_table_lookup(
        cluster_tree, node_to_leaf, pll_obj,
        AB_tables_lvl, L2B, L2C, leaf_row_index,
        gt, preprocessing_time
    )
    print("\n=== Table-only LCA Query Evaluation (All-Table, strict lookup) ===")
    print(df_eval.to_string(index=False))
    print(f"\n[Summary] Preprocessing={preprocessing_time:.3f}s, parents_with_AB={len(AB_tables_lvl)}, parents_with_L2={len(L2B)}")
    if idx_stats:
        total_nodes = sum(s["index_nodes"] for s in idx_stats.values())
        total_edges = sum(s["index_edges"] for s in idx_stats.values())
    ...
    print("\n[OK] Finished: strict table lookup with supergraph-based hierarchical tree.")
