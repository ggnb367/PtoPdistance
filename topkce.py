...
def payload_bytes_edges(edge_count: int) -> int:
    return (8 + 8 + 8) * edge_count

def payload_bytes_nodes(node_count: int) -> int:
    return 16 * node_count
...
def build_g_out_and_outpll(G, partition, clusters, boundary_sets, boundary_pairs):
    ...
    # outside PLL — degree descending order
    t1 = time.time()
...
# ===============================
# New C (C2): per-source Dijkstra on G_rep, keep Top-K (S->B) paths
# ===============================
...
def build_c2_pairs_bfs_like(G_rep, boundary_sets, partition, topk_pairs=16, max_workers=None):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in tqdm(as_completed(futures), total=len(futures),
                        desc="Hybrid C2: per-source Dijkstra (TopK per (S,B))", unit="cluster"):
            S, perB = fut.result()
...
def build_node_to_all_boundary_arrays(inside_pll, clusters, boundary_sets, max_workers=None):
    ...
    tasks = []
    for cid, pll in inside_pll.items():
        blist = sorted(boundary_sets.get(cid, set()))  # 全量边界口
        allb_list[cid] = blist
    ...
    stats = {
        ...
        "entries": total_entries,
        "payload": payload_bytes_floats(total_entries),
        "all_boundary_counts": {cid: len(allb_list[cid]) for cid in allb_list},
    }
    return node_to_allb_arr, allb_list, allb_index, stats


def build_ab_topk_from_boundary_to_cluster(boundary_to_cluster_D, partition, boundary_sets, topk_mid=16):
    """
    基于 表-1（b→B 的中段距离）聚合得到 (A,B) 维度的中段 Top-K 候选
    ...
    for b, perB in boundary_to_cluster_D.items():
        ...
        items_by_B = defaultdict(list)
        for B, (mid, c) in perB.items():
            items_by_B[B].append((mid, b, c))

        for B, items in items_by_B.items():
            items.sort(key=lambda x: x[0])
            keep = items[:topk_mid]
            for mid, bb, c in keep:
                AB_topk[A][B].append((bb, mid, c))

    for A in AB_topk:
        for B in AB_topk[A]:
            AB_topk[A][B].sort(key=lambda x: x[1])
            AB_topk[A][B] = AB_topk[A][B][:topk_mid]

    kept = sum(len(m) for A in AB_topk for m in AB_topk[A].values())
    stats = {
        "kept_total": kept,
        "payload": payload_bytes_triple(kept)
    }
    return AB_topk, stats


def query_e_twotables(u, v, node_cluster,
    ...
    """
    E 的查表版（修复版）：
    - 左端：u→b 用“全边界数组”取最近 topk_e_u（用 allb_list 的顺序做反查！）
    - 中段：优先用 AB_topk[(A,B)] 里的 b；若缺失则回退到 boundary_to_cluster_D[b][B]
    - 右端：c→v 直接用 B 簇的全边界数组查
    """
    A = node_cluster[u]
    B = node_cluster[v]
    if A == B:
...
def query_hybrid_c2_pairs_usedarrays(u, v, node_cluster,
...
def evaluate(G,
             ...
             # E(two-table):
             node_to_allb_arr_E, allb_index_E, allb_list_E, AB_topk, topk_e_u, topk_e_mid,
             n_pairs=500):
    nodes = list(G.nodes())
    ...
    t0 = time.time()
    for u, v, d in gt:
        est = query_hybrid_c2_pairs_usedarrays(u, v, node_cluster,
                                               C2_pairs,
                                               node_to_used_arr_C2, used_index_C2,
                                               inside_pll)
        if est == d:
        ...
        if est != float("inf"):
            err += abs(est - d); total += 1
    tC2 = time.time() - t0
    rows.append(["Hybrid C (TopK_AB routes + USED-arrays for used boundaries only)", tC2, total, correct, (err/total if total>0 else float("inf"))])
    ...
    t0 = time.time()
    for u, v, d in gt:
        est = query_e_twotables(u, v, node_cluster,
                                node_to_allb_arr_E, allb_index_E, allb_list_E,
                                boundary_to_cluster_D, AB_topk,
                                topk_e_u=topk_e_u, topk_e_mid=topk_e_mid,
                                inside_pll=inside_pll)
        if est == d:
            correct += 1
        if est != float("inf"):
            err += abs(est - d); total += 1
    tE = time.time() - t0
    rows.append([f"Hybrid E (Two-Table FIXED: u-TopK({topk_e_u}) × mid-TopK({topk_e_mid}) + table lookups)", tE, total, correct, (err/total if total>0 else float("inf"))])
...
def build_all(G, args):
    ...
    # 只对“被用到的边界口”造表
    node_to_used_arr_C2, used_b_list_C2, used_b_index_C2, stats_C2_used = build_node_to_boundary_used_arrays(
    ...
    # --------- E (Two-Table) phase ----------
    # 表-2：全量点→边界数组
    node_to_allb_arr_E, allb_list_E, allb_index_E, stats_E_allb = build_node_to_all_boundary_arrays(
        inside_pll, clusters, boundary_sets, max_workers=args.d_workers
    )
    # (A,B) 中段 TopK（由表-1汇总）
    AB_topk, stats_AB_topk = build_ab_topk_from_boundary_to_cluster(
        boundary_to_cluster_D, partition, boundary_sets, topk_mid=args.topk_e_mid
    )
    ...
    stats = {
        ...
        # E Two-Table
        "e_allb_arrays": {"time_sec": stats_E_allb["time_sec"],
                          "entries": stats_E_allb["entries"],
                          "payload": stats_E_allb["payload"],
                          "all_boundary_counts": stats_E_allb["all_boundary_counts"]},
        "e_ab_topk": {"kept_total": stats_AB_topk["kept_total"],
                      "payload": stats_AB_topk["payload"]},
    }
    ...
    stats["totals"] = {
        ...
        "build_E_sec": stats["inside"]["time_sec"] + stats["g_out"]["time_sec"]
                       + stats["d_potentials"]["time_sec"] + stats["e_allb_arrays"]["time_sec"],
        ...
        "payload_E_bytes": stats["inside"]["payload"] + stats["g_out"]["payload_nodes"] + stats["g_out"]["payload_edges"]
                           + stats["e_allb_arrays"]["payload"] + stats["e_ab_topk"]["payload"],
    }
    ...
    return (partition, clusters, inside_pll, boundary_sets, node_cluster,
            ...
            node_to_topk_D, boundary_to_cluster_D,
            # E two-table
            node_to_allb_arr_E, allb_list_E, allb_index_E, AB_topk,
            stats)
...
def print_build_stats(stats):
    ...
    rows = [
        ...
        [f"C (new): per-source Dijkstra (TopK_AB, k={stats['c2_pairs']['topk_pairs']})", stats["c2_pairs"]["time_sec"]],
        ["C (new): Build node→USED-boundary arrays", stats["c2_used_arrays"]["time_sec"]],
        ...
        ["E(two-table): Build node→ALL-boundary arrays", stats["e_allb_arrays"]["time_sec"]],
        ["E(two-table): Build AB mid-TopK from potentials", 0.0],
        ...
        ["Total Build E(two-table)", stats["totals"]["build_E_sec"]],
    ]
    ...
    rows2 = [
        ...
        [f"C(new): Kept (A,B) entries (TopK_AB)", stats["c2_pairs"]["kept_total"], format_mib(stats["c2_pairs"]["payload"])],
        ["C(new): Node→USED-boundary entries", stats["c2_used_arrays"]["entries"], format_mib(stats["c2_used_arrays"]["payload"])],
        ...
        ["E(two-table): Node→ALL-boundary entries", stats["e_allb_arrays"]["entries"], format_mib(stats["e_allb_arrays"]["payload"])],
        ["E(two-table): AB mid-TopK kept", stats["e_ab_topk"]["kept_total"], format_mib(stats["e_ab_topk"]["payload"])],
        ...
        ["Total payload  [E(two-table)]", "", format_mib(stats["totals"]["payload_E_bytes"])],
    ]
...
def main():
    ...
    parser.add_argument("--pairs", type=int, default=1500, help="number of query pairs for evaluation")
    ...
    parser.add_argument("--d_workers", type=int, default=None, help="parallel workers for D/E (potentials & arrays)")

    # Top-K
    parser.add_argument("--topk_c_pairs", type=int, default=16, help="Top-K kept routes per (source, target) pair in C (new C2)")
    parser.add_argument("--topk_d_u", type=int, default=16, help="Top-K personalized exits per node for D(original)")
    # E(two-table) 的两个 TopK
    parser.add_argument("--topk_e_u", type=int, default=16, help="E(two-table): Top-K nearest exits from u (left side)")
    parser.add_argument("--topk_e_mid", type=int, default=16, help="E(two-table): Top-K mid candidates per (A,B) aggregated from potentials")
    ...
    df_eval = evaluate(
        ...
        node_to_topk_D, boundary_to_cluster_D,
        # E(two-table):
        node_to_allb_arr_E, allb_index_E, allb_list_E, AB_topk, args.topk_e_u, args.topk_e_mid,
        n_pairs=args.pairs
    )
