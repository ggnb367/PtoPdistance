...
def _c2_per_source_worker(args):
    """
    From ("VIRT", S) run Dijkstra on G_rep (weighted), forbidding other virtual nodes.
    For each reached external boundary node t in B!=S:
      keep Top-K per (S,B): (exit_b, t, dist_mid), ordered by dist_mid.
    Returns (S, {B: [(b_exit, t_entry, dist_mid), ...]})
    """
...
def build_c2_pairs_bfs_like(G_rep, boundary_sets, partition, topk_pairs=16, max_workers=None):
    """
    For each S, run weighted Dijkstra from ("VIRT", S), forbid other virtual nodes.
    Keep Top-K per (S,B): (b_exit, t_entry, dist_mid).
    Returns:
      C2_pairs: dict A->{B: [(b, t, dist_mid), ...]}
      used:     dict cid->set(boundaries) used on both sides
      stats:    timing / counts / payload estimates
    """
...
# ===============================
# D (original): node→TopK(u) personalized exits
# ===============================
...
def query_hybrid_c2_pairs_usedarrays(u, v, node_cluster,
    ...
    """
    New C method query:
      候选三元组顺序为 (dist_mid, b_exit, t_entry)。
      对跨簇 (u in A, v in B):
        du = dist(u->b_exit)  [回表: node_to_used_arr_C2[A][u][idxA[b_exit]]
        dv = dist(t_entry->v) [回表: node_to_used_arr_C2[B][v][idxB[t_entry]]
        估计 = dist_mid + du + dv
    """
    A = node_cluster[u]
    B = node_cluster[v]
    ...
    best = float("inf")
    for (mid, b, t) in cand:  # <-- 修正：先 mid，再 b、t
...
# ===============================
# E: helper to build D-pairs and USED arrays for E
# ===============================
...
def evaluate(G,
    ...
    for u, v, d in gt:
        ...
        if est != float("inf"):
            err += abs(est - d); total += 1
    tC2 = time.time() - t0
    rows.append(["Hybrid C (per-source Dijkstra TopK_AB + USED arrays)", tC2, total, correct, (err/total if total>0 else float("inf"))])
