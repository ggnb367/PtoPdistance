# -*- coding: utf-8 -*-
"""
gtree_m1.py  (CC-parallel HL + level=-2 SUPER_ROOT + level=-1 CC_ROOTs)

新增/变更要点（本版）：
A) 不同连通分量（CC）并行化：
   - 在分层Leiden前先切CC；
   - 每个CC在独立进程里跑 hierarchical_leiden（_hl_on_cc_worker）。

B) 分层结构改造：
   - 为每个CC创建 level=-1 的 CC_ROOT（孩子只属于该CC）；
   - 再创建一个 level=-2 的 SUPER_ROOT 连接所有 CC_ROOTs；
   - **不在 level=-2 上构建代表图与跨表**（完全跳过）；
   - 查询时若 u、v 不在同一CC，**直接返回 inf**（早退，不触发 level=-2）。

C) 知识图谱构图（关键改动）：
   - 对 WN18：不再创建 ("REL", i, r) 关系节点；
   - 每条 (h, r, t) 收缩为一条无向的实体边 (h, t)，多关系合并为一条边：
     * 边权 weight = 该实体对所有关系代价的最小值（默认为 1.0，对应旧法 0.5+0.5）；
     * 以边属性保留语义：rels（所有关系的集合）、w_by_rel（关系→权重）、multi（关系数量）。

D) 其余（保持你之前的规则）：
   - 叶子：PLL + TopK + D_pp(用PLL精确) + D_np；
   - 非叶代表域：rep_nodes = (子簇之间真实跨边端点) ∪ c.global_borders；
     child_portals[ch] = (parent.rep_nodes ∩ ch.nodes)；
   - 非叶抬升/跨子簇：**一次**加权 multi-source Dijkstra。
"""
...
# ----------------------------- utils -----------------------------
def is_rel(n) -> bool:
    # 兼容旧接口：当前 WN18 构图已不再产生 ("REL", i, r) 这样的关系节点
    return isinstance(n, tuple) and len(n) > 0 and n[0] == "REL"

def node_sort_key(n):
    # 确保 str / int / ("REL", i, r) 可比较
    if is_rel(n):
        try:
            _, idx, rel = n
...
def load_wn18_graph_relation_nodes(path: str) -> nx.Graph:
    ...
    # 若需要为不同 relation 设定不同代价，可在此处覆盖
    rel_weight: Dict[Any, float] = {}  # 默认均 1.0（对应旧法 0.5+0.5）
...
def _hl_on_cc_worker(args):
    """
    在子进程内对一个 CC 诱导子图跑 hierarchical_leiden。
...
class HierRepHybridTopKPLL:
    def __init__(self, G: nx.Graph,
        ...
        # 3) 自底向上：父层代表图 + multi-source cross tables
        print("[Stage] bottom-up representative graphs (closure rule + global_borders) + multi-source cross tables ...")
    ...
    def _build_hierarchy_with_cc_roots(self, hl_list, components) -> Tuple[Cluster, List[Cluster], Dict[Any, Cluster]]:
        ...
        # 为每个 CC 创建一个 CC_ROOT（level=-1），把该 CC 中所有“没有父”的 level=0 簇都挂到它下面
        ...
        # SUPER_ROOT（level=-2），挂所有 CC_ROOT
    ...
    # ---------- 父层代表图 + multi-source cross tables ----------
    def _build_rep_graphs_and_cross_tables_multisource(self):
        ...
        for lvl in levels:
            ...
            for c in tqdm(Cs, desc=f"rep-graph build (level={lvl})", unit="cluster"):
                self._build_single_rep_graph_new_rule(c)

            # 再基于 multi-source 生成 cross tables
            for c in tqdm(Cs, desc=f"cross tables (multi-source) (level={lvl})", unit="cluster"):
                if c.is_leaf:
                    continue
                self._compute_parent_cross_tables_multisource(c)

    def _build_single_rep_graph_new_rule(self, c: Cluster):
        # 叶子：代表域=叶门户；边=叶 D_pp 完全成团
        if c.is_leaf:
        ...
        # level=-2 已在上层跳过；这里最多 level=-1（每个 CC 的根）或更深
        children = c.children
        ...
        # rep_nodes = (跨边端点) ∪ c.global_borders
        ...
        try:
            ...
            tqdm.write("[parent] level={} cid={} children={} rep_nodes={}".format(c.level, c.cid, len(children), len(c.rep_nodes)))
        ...
        for ch in children:
            ...
            if ch.is_leaf:
                ...
                if not good:
                    c.child_portal_idx_in_child[ch] = []
                    continue
                ports2, idx_child2 = zip(*good)
                ports = list(ports2); parent_idx = [c.idx_portal_in_rep[p] for p in ports]
                c.child_portals[ch] = ports
                c.child_portal_idx_in_parent[ch] = parent_idx
                c.child_portal_idx_in_child[ch] = list(idx_child2)

                # 叶子闭包：用 D_np 的节点行（门户自身）取门户↔门户
                for a in range(len(ports)):
                    pa = ports[a]
                    ia_parent = parent_idx[a]
                    ra = ch.idx_node_in_leaf.get(pa, None)
                    if ra is None:
                        continue
                    for b in range(a+1, len(ports)):
                        pb = ports[b]
                        cb = ch.idx_portal_in_leaf.get(pb, None)
                        if cb is None:
                            continue
                        w = float(ch.D_np[ra, cb])
                        if math.isinf(w):
                            continue
                        ib_parent = parent_idx[b]
                        c.rep_edges.append((ia_parent, ib_parent, w))
            else:
                ...
                if not good:
                    ...
                    continue
                ports2, idx_child2 = zip(*good)
                ports = list(ports2); idx_child = list(idx_child2)
                parent_idx = [c.idx_portal_in_rep[p] for p in ports]
                c.child_portals[ch] = ports
                c.child_portal_idx_in_parent[ch] = parent_idx
                ...
                if ch.D_pp is not None:
                    for a in range(len(ports)):
                        ia_child = idx_child[a]
                        ia_parent = parent_idx[a]
                        for b in range(a+1, len(ports)):
                            ib_child = idx_child[b]
                            ib_parent = parent_idx[b]
                            w = float(ch.D_pp[ia_child, ib_child])
                            if math.isinf(w):
                                continue
                            c.rep_edges.append((ia_parent, ib_parent, w))
                else:
                    ...
                    for a in range(len(ports)):
                        s = idx_child[a]
                        ia_parent = parent_idx[a]
                        dist = np.full(Pch, np.inf, dtype=np.float32)
                        dist[s] = 0.0
                        pq = [(0.0, s)]
                        while pq:
                            d,u = heapq.heappop(pq)
                            if d > float(dist[u]):
                                continue
                            for v2,w in adj_ch[u]:
                                nd = d + w
                                if nd < float(dist[v2]):
                                    dist[v2] = np.float32(nd)
                                    heapq.heappush(pq, (nd, v2))
                        for b in range(a+1, len(ports)):
                            ib_child = idx_child[b]
                            ib_parent = parent_idx[b]
                            w = float(dist[ib_child])
                            if math.isinf(w):
                                continue
                            c.rep_edges.append((ia_parent, ib_parent, w))

        # 跨子簇原始边：两端都在 c.rep_nodes 内再保留
    ...
    def query(self, u: Any, v: Any) -> float:
        ...
        # 快速早退：不同 CC 直接返回 inf（无需触发 level=-2 超根）
        ...
        L = self._lca(Cu, Cv)
        # 如果 L 是 SUPER_ROOT（level=-2），由于不同CC已早退，这里通常不会进来；
        # 但若进来也不在 -2 上做代表图：直接返回 inf。
        if L.level == -2:
            return float("inf")
...
def main():
    ...
    parser.add_argument("--eval_pairs", type=int, default=50)
    parser.add_argument("--max_cluster_size", type=int, default=2000)
    parser.add_argument("--resolution", type=float, default=0.2)
    parser.add_argument("--random_seed", type=int, default=42)
    parser.add_argument("--leaf_topk", type=int, default=16)
