...
# ===============================
# Wikidata5M Loader -> NetworkX
# ===============================
def load_graph_from_wikidata5m(setting="transductive", root=None, max_edges=None):
    """
    用 PyG 的 Wikidata5M 下载/解析三元组，构建无向加权图。
    - setting: 'transductive' 或 'inductive'
    - max_edges: 仅取前 N 条边（按文件顺序），控制规模；None 则使用全部
    """
    root = root or os.path.abspath("./data/wikidata5m")
    ds = Wikidata5M(root=root, setting=setting, force_reload=True)
    data = ds[0]

    ei = data.edge_index  # shape [2, E]
    if max_edges is not None:
        max_edges = min(max_edges, ei.size(1))
        ei = ei[:, :max_edges]

    src = ei[0].cpu().numpy()
    dst = ei[1].cpu().numpy()

    # 合并多关系到同一无向边（计数作权重也可；这里统一设为1.0）
    G = nx.Graph()
    # 注意：实体ID是0..N-1 的连续整数，直接当作节点即可
    num_nodes = int(max(src.max(), dst.max())) + 1 if ei.numel() > 0 else 0
...
# ===============================
# Hybrid B: cluster-pair shortest on virtual graph
# ===============================
...
# ===============================
# C worker: per-source SSSP on G_rep, Top-K per (S,B)
# ===============================
...
def build_all(G, args):
    ...
    # E: 复用 D 的 potentials，按 TopK_AB 选对并构 USED arrays
...
def main():
    ...
    # Wikidata5M source
    parser.add_argument("--wikidata_setting", type=str, default="transductive",
                        choices=["transductive", "inductive"])
    parser.add_argument("--kg_root", type=str, default=None, help="Wikidata5M 数据根目录（默认 ./data/wikidata5m）")
    parser.add_argument("--max_edges", type=int, default=None, help="仅取前 N 条边采样子图")
    # Eval
    ...
    # Load KG graph from Wikidata5M
    G = load_graph_from_wikidata5m(args.wikidata_setting, root=args.kg_root, max_edges=args.max_edges)
    print(f"[INFO] Wikidata5M({args.wikidata_setting}) graph: |V|={G.number_of_nodes()}, |E|={G.number_of_edges()}")
